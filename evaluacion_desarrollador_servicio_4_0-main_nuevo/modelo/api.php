	...
	class api
	{
		...
		public function MetodoGet(){			
			try {
				...
				$Validar = new valida();
				//Se le asignó el valor del objeto trayéndolo desde objeto.php
		...
		public function exportar($nombreArchivo){
			try{
				...
				$ObjetoColor = new objeto();
				$rutatemp = "temp/";
				//De igual modo de arriba se le asignó el valor del objeto
				$Valor = $ObjetoColor ->ObtenerObjeto();

				$nombreArchivo = $nombreArchivo . ".json";
				file_put_contents($rutatemp . $nombreArchivo, json_encode($Valor), FILE_APPEND | LOCK_EX);
